...
const LABELS: Record<BackupCategory, string> = {
  conversations: "聊天與跨裝置紀錄",
  memories: "記憶、關係與舊日記",
  planning: "任務、生活與使用量歷史",
  personalization: "個人化、通知與 Discord 資料",
  knowledge: "知識庫",
  settings: "全部設定（不含密鑰）",
};
...
const CATEGORIES = Object.keys(PATHS) as BackupCategory[];
const MAX_FILES = 10_000;
const MAX_BYTES = 512 * 1024 * 1024;

interface BackupFile { category: BackupCategory; path: string; size: number; sha256: string; data: string }
interface BackupBundle { format: "cyrene-backup"; version: 1; createdAt: string; appVersion: string; categories: BackupCategory[]; files: BackupFile[] }
export interface BackupSummary { filePath: string; createdAt: string; appVersion: string; categories: Array<{ id: BackupCategory; label: string; fileCount: number; sizeBytes: number }>; fileCount: number; sizeBytes: number }
export interface BackupConfig { autoEnabled: boolean; retentionDays: 7 | 30; lastAutoBackupAt?: string }
...
function isCategory(value: unknown): value is BackupCategory {
  return typeof value === "string" && CATEGORIES.includes(value as BackupCategory);
}
...
function parseBundle(filePath: string): BackupBundle {
  const raw = zlib.gunzipSync(fs.readFileSync(filePath), { maxOutputLength: MAX_BYTES });
  ...
  for (const file of bundle.files) {
    if (!file || !isCategory(file.category)) throw new Error("備份分類無效");
...
function summarize(bundle: BackupBundle, filePath: string): BackupSummary {
  return {
    filePath, createdAt: bundle.createdAt, appVersion: bundle.appVersion,
    categories: bundle.categories.map((id) => {
      const files = bundle.files.filter((file) => file.category === id);
      return { id, label: LABELS[id], fileCount: files.length, sizeBytes: files.reduce((sum, file) => sum + file.size, 0) };
    }),
...
export class BackupManager {
  ...
  getConfig(): BackupConfig {
    ...
    } catch { return { autoEnabled: false, retentionDays: 7 }; }
  }
  ...
  create(filePath: string, requested: unknown): BackupSummary {
    const categories = Array.isArray(requested) ? requested.filter(isCategory) : CATEGORIES;
    if (!categories.length) throw new Error("請至少選擇一種資料");
    const seen = new Set<string>();
    const files: BackupFile[] = [];
    let total = 0;
    for (const category of categories) for (const entry of PATHS[category]) for (const relative of listFiles(this.userData, entry)) {
      if (seen.has(relative)) continue;
      seen.add(relative);
      let bytes = fs.readFileSync(path.join(this.userData, relative));
      if (category === "settings" && relative.endsWith(".json")) {
        try { bytes = Buffer.from(JSON.stringify(redactSecrets(JSON.parse(bytes.toString("utf8"))), null, 2)); } catch { /* 保留格式異常的舊檔 */ }
      }
      total += bytes.length;
      if (files.length >= MAX_FILES || total > MAX_BYTES) throw new Error("備份內容超過 512 MB 安全限制");
      files.push({ category, path: relative.replace(/\\/g, "/"), size: bytes.length, sha256: createHash("sha256").update(bytes).digest("hex"), data: bytes.toString("base64") });
    }
  ...
  restore(filePath: string, requested: unknown): { restoredFiles: number; safetyBackupPath: string } {
    ...
    const categories = Array.isArray(requested) ? requested.filter(isCategory) : bundle.categories;
    ...
    for (const file of bundle.files) {
      ...
      if (file.category === "settings" && file.path.endsWith(".json") && fs.existsSync(destination)) {
        try { bytes = Buffer.from(JSON.stringify(preserveCurrentSecrets(JSON.parse(bytes.toString("utf8")), JSON.parse(fs.readFileSync(destination, "utf8"))), null, 2)); } catch { /* 使用原始備份 */ }
      }
  ...
  runAutoBackupIfDue(): BackupSummary | null {
    ...
    if (!config.autoEnabled || Date.now() - Date.parse(config.lastAutoBackupAt ?? "1970-01-01") < 20 * 60 * 60 * 1000) return null;
    const now = new Date();
    const result = this.create(path.join(this.backupsDir, `自動-${now.toISOString().slice(0, 10)}.cybackup`), CATEGORIES);
    atomicWrite(this.configPath, JSON.stringify({ ...config, lastAutoBackupAt: now.toISOString() }, null, 2));
    const cutoff = Date.now() - config.retentionDays * 86_400_000;
    if (fs.existsSync(this.backupsDir)) for (const name of fs.readdirSync(this.backupsDir)) {
      const file = path.join(this.backupsDir, name);
      try { if (name.startsWith("自動-") && name.endsWith(".cybackup") && fs.statSync(file).mtimeMs < cutoff) fs.unlinkSync(file); } catch { /* best effort */ }
    }
    return result;
  }
}
