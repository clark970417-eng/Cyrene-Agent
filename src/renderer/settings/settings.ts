...
async function loadConfig(): Promise<void> {
  try {
    ...
    if (apiModeEnabledInput) {
      const isWebLlm = loadedProvider === "chatgpt_web" || loadedProvider === "gemini_web" || (cfg as any).apiModeEnabled === false;
...
apiModeEnabledInput?.addEventListener("change", async () => {
  ...
  try {
    if (!apiModeEnabledInput.checked) {
      ...
      applyPreset("chatgpt_web", "GPT-4o", "", "https://chatgpt.com", "ChatGPT Plus (網頁版 · GPT-4o)", "auto");
    } else {
      // 開啟 API 模式 ➔ 切換至 OpenRouter 免費 API 模式
      applyPreset("Custom", "openrouter/free", "", "https://openrouter.ai/api/v1", "OpenRouter Free", "auto");
    }
    await persistApiSettings();
    setSaveStatus("已保存", "is-ok");
  } catch {
